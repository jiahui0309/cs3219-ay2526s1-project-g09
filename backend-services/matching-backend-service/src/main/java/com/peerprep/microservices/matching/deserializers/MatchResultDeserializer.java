 ...
 * 
 * Ensures that the required fields {@code user1RequestId}, {@code user2RequestId}, {@code user1Preference}, and
 * {@code user2Preference} are present in the JSON input. If any field is missing or {@code null}, a
 * {@link JsonMappingException} is thrown with details of the invalid input.
 *
 */
public class MatchResultDeserializer extends StdDeserializer<MatchingNotification> {
  ...
  /**
   * Extracts and deserializes a required {@link UserPreference} field from the JSON node.
   *
   * @param jp the JSON parser
   * @param node the JSON object node
   * @param fieldName the field name to extract
